...
async def forget_resetpassword():
    ...
    #이메일 인증키 체크
    try:
        redis_key_bytes = await glob.redis.get(f"guweb:ForgotEmailVerify:{email}")
        ...
        return await flash('error', 'Email verification code is invalid.', 'forgot')
...
async def settings_profile_post():
    ...
    #old 이메일 인증키 체크
    try:
        redis_key_bytes = await glob.redis.get(f"guweb:Settings/profile:{old_email}")
        if redis_key_bytes is None:
            return await flash('error', 'old Email verification code is Expired.', 'settings/profile')
        RedisKEY = redis_key_bytes.decode("utf-8")
    except (AttributeError, UnicodeDecodeError) as e:
        log(f'Redis key decode error: {e}', Ansi.LRED)
        return await flash('error', 'old Email verification code is invalid.', 'settings/profile')
    ...
    if new_email != old_email:
        #new 이메일 인증키 체크
        try:
            redis_key_bytes = await glob.redis.get(f"guweb:Settings/profile:{old_email}->{new_email}")
            if redis_key_bytes is None:
                return await flash('error', 'new Email verification code is Expired.', 'settings/profile')
            RedisKEY = redis_key_bytes.decode("utf-8")
        except (AttributeError, UnicodeDecodeError) as e:
            log(f'Redis key decode error: {e}', Ansi.LRED)
            return await flash('error', 'new Email verification code is invalid.', 'settings/profile')
